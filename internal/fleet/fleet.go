...
const (
	LocalHost     = "local"
	LiveElsewhere = "elsewhere"  // handed off; the named host has not been asked (or was unreachable)
	LiveStale     = "elsewhere?" // handed off, but the named host answered without the task
...
func Decode(host string, raw []byte) ([]Row, error) {
	...
	for _, r := range env.Tasks {
		if r.Task == nil || r.HandedOffTo != nil {
			continue
		}
		r.Host = host
		rows = append(rows, r)
...
// HandedOffTo pointer (the row's `live` says elsewhere / elsewhere?).
func Merge(local []Row, tombstones []*state.Task, results []Result) (rows []Row, warnings []string) {
	...
	for _, t := range tombstones {
		...
		row := Tombstone(t)
		if answered[t.HandedOffTo.Host] {
			row.Live = LiveStale
		}
...
// Elsewhere renders the tombstone line: `grove-142 ⇢ grove-host  <branch>
// handed off 2h ago` (the `?` suffix marks a stale tombstone). ago is the
// caller's age formatter so gv ls and the cockpit print the same unit.
func Elsewhere(r Row, ago func(time.Time) string) string {
	...
	return fmt.Sprintf("%s ⇢ %s  %s  handed off %s ago", r.Ticket, host, h.Branch, ago(h.At))
}
