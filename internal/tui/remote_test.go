...
func remoteTestModel(t *testing.T) Model {
	...
	tomb := &state.Task{Ticket: "gr-2", Repo: "grove", Done: true, Agent: state.AgentIdle, Created: at,
		HandedOffTo: &state.Handoff{Host: "vps", Branch: "gr-2-work", At: at}}
...
func TestRemoteToggleFetchesOnceAndMerges(t *testing.T) {
	...
	if v := m.View(); !strings.Contains(v, "gr-2 ⇢ vps ") || !strings.Contains(v, "handed off") {
	...
	if len(m.tasks) != 2 || m.tasks[1].HandedOffTo != nil || !fleet.IsRemote(m.tasks[1]) {
