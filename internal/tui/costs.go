...
func (m Model) viewCosts() string {
	...
	rows := []string{sHeaderCol.Render(truncPad("   "+pad("TICKET", m.ticketColWidth())+pad("EST $", 9)+pad("TURNS", 7)+pad("IN", 9)+pad("OUT", 9)+pad("CACHE%", 8)+pad("MODELS", 20)+"TITLE", w))}
	...
	for _, r := range m.costs.rows {
		line := "   " + pad(r.ticket, m.ticketColWidth()) +
			...
			sDelivery.Render(pad(r.tot.Mix(), 20)) +
