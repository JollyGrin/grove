...
func (m *Model) assemble() {
	...
	for _, r := range rows {
		...
		if fleet.IsRemote(r.Task) || r.HandedOffTo != nil {
...
func (m Model) handleKey(k tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	if t := m.selected(); t != nil && (fleet.IsRemote(t) || t.HandedOffTo != nil) {
		switch k.String() {
		case "enter", "n", "a", "v", "d":
...
func elsewhereFlash(t *state.Task) string {
	if t.HandedOffTo != nil {
		return t.Ticket + " was handed off to " + t.HandedOffTo.Host + " — steer it there"
	}
