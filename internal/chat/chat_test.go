...
func TestLessOrdersByActivity(t *testing.T) {
	...
	cases := []struct {
		...
		{"activity outranks chat number (grove-302)",
			Row{Kind: KindChat, N: 1, LastActive: old},
			Row{Kind: KindChat, N: 2, LastActive: recent}, false},
		{"…and the number breaks an activity tie",
			Row{Kind: KindChat, N: 1, LastActive: mid},
			Row{Kind: KindChat, N: 2, LastActive: mid}, true},
